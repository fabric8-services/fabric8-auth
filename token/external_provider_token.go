...
type ExternalProviderToken struct {
	...
	ExternalProviderID uuid.UUID `sql:"type:uuid"`
	Token              string
	Scope              string
...
// ExternalProviderTokenFilterByExternalProviderID is a gorm filter by 'profile_url'
func ExternalProviderTokenFilterByExternalProviderID(externalProviderID uuid.UUID) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		return db.Where("external_provider_id = ?", externalProviderID).Limit(1)
	}
}
