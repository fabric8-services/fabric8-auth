...
func (rest *TestTokenREST) UnSecuredController() (*goa.Service, *TokenController) {
	...
	loginService.KeycloakOAuthProvider = *login.NewKeycloakOAuthProvider(rest.Application.Identities(), rest.Application.Users(), testtoken.TokenManager, rest.Application, profileService, &testsupport.DummyOSORegistrationApp{})
...
func (s *DummyKeycloakOAuthService) ExchangeRefreshToken(ctx context.Context, refreshToken string, serviceConfig login.Configuration) (*token.TokenSet, error) {
	if s.exchangeStrategy == "401" {
		return nil, errors.NewUnauthorizedError("failed")
	}
