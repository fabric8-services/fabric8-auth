...
. Client sends a generated state. Auth "proxies" it to OAuth Service and thereby initiates the first request in oauth2
. OAuth Service redirects to IdP etc. User logs in..
. OAuth Service calls back Auth with a code
