...
== OpenID support in Auth API

=== To get OpenID Configuration of Fabric8-Auth Service
...
We have created an OpenID Compliant UserInfo endpoint (`/api/userinfo`) which would return stardard openid claims. The list of claims may increase as per the requirements of OpenID Clients ( Different services which are using Auth service).
