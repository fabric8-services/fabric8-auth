...
The signature has to be verified against the public key of the corresponding private key used to sign the JWT. The list of public keys in use are available at https://auth.openshift.io/api/token/keys?format=pem

====== Golang
Import `github.com/fabric8-services/fabric8-auth/token/jwk` to make use of `FetchKeys(endpointURL)` from https://github.com/fabric8-services/fabric8-auth/blob/master/token/jwk/fetch_keys.go

==== Using the right public key

You could attempt verification of the signature against all the public keys returned in https://auth.openshift.io/api/token/keys?format=pem . If the verification succeeds for any one of the public keys, you could consider the token to be valid.

Alternatively, you could choose the public key which has a "kid" which matches the "kid" claim in the token.
