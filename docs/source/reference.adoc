...
. Client sends a generated state. Auth "proxies" it to KC and thereby initiates the first in oauth2
. KC redirects to IdP etc. User logs in..
. KC calls back Auth with a code
