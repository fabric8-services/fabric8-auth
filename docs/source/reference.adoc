...
Alternatively, you could attempt verification of the signature against all the public keys returned in https://auth.openshift.io/api/token/keys?format=pem . If the verification succeeds for any one of the public keys, you could consider the token to be valid.
