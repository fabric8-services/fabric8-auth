...
When an access token is required in automated tests it can be tricky to get such token via regular login workflow
because login flow requires user interaction in a browser. Using an offline tokens is the preferable way to get an access token in automated tests.

Offline token is a special refresh token which does not expire and can be stored in a test as a secret.

==== How to get Offline Token

1. Open in our browser: https://auth.openshift.io/api/login/authorize?scope=offline_access&redirect=https%3A%2F%2Fauth.openshift.io%2Fapi%2Fstatus
Browser will be redirected to https://developers.redhat.com where you need to login using your test account credentials. If you are not asked for credentials then it means you are already logged in (maybe as another user if you have a few RHD accounts). Re-login if you want to login as different user (logout then login again).
...
==== How to use Offline Token

Before run a test obtain a new access token. Please do not obtain a new access token multiple times during the test
because generating a new token is an expensive operation. Get access token in the beginning of the test  be Every time
you and then re-use it:

[source]
curl -H "Content-Type: application/json" -X POST -d '{"refresh_token":"<REFRESH_TOKEN>"}' https://auth.openshift.io/api/token/refresh

As a result you will get a JSON with access token which you can use in your test.