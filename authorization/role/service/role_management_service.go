...
// RoleManagementService defines the contract for managing role assignments for a resource
...
func NewRoleManagementService(repositories repository.Repositories, db *gorm.DB) *RoleManagementServiceImpl {
	return &RoleManagementServiceImpl{repo: repositories, db: db}
}
...
func (r *RoleManagementServiceImpl) ListByResource(ctx context.Context, resourceID string) ([]rolerepo.IdentityRole, error) {
	defer goa.MeasureSince([]string{"goa", "db", "role_management_service", "ListByResource"}, time.Now())
	var identityRoles []rolerepo.IdentityRole

	err := r.repo.ResourceRepository().CheckExists(ctx, resourceID)
	if err != nil {
...
func (r *RoleManagementServiceImpl) ListAvailableRolesByResourceType(ctx context.Context, resourceType string) ([]role.RoleScope, error) {
	defer goa.MeasureSince([]string{"goa", "db", "role_management_service", "ListAvailableRolesByResourceType"}, time.Now())
	var roleScopes []role.RoleScope

	_, err := r.repo.ResourceTypeRepository().Lookup(ctx, resourceType)
	if err != nil {
		log.Error(ctx, map[string]interface{}{
...
func (r *RoleManagementServiceImpl) ListByResourceAndRoleName(ctx context.Context, resourceID string, roleName string) ([]rolerepo.IdentityRole, error) {
	defer goa.MeasureSince([]string{"goa", "db", "role_management_service", "ListByResourceAndRoleName"}, time.Now())
	var identityRoles []rolerepo.IdentityRole
