...
type InvitationModelService interface {
	Issue(ctx context.Context, repo repository.Repositories, issuingUserId uuid.UUID, inviteTo string, invitations []invitation.Invitation) error
}
...
func NewInvitationModelService(permissionSvc permissionModel.PermissionModelService) InvitationModelService {
...
func (s *GormInvitationModelService) Issue(ctx context.Context, repo repository.Repositories, issuingUserId uuid.UUID, inviteTo string,
	...
	if err != nil {
		...
		inviteToResource, err = repo.ResourceRepository().Load(ctx, inviteTo)
		if err != nil {
	...
	if inviteToResource == nil {
		...
		inviteToIdentity, err = repo.Identities().Load(ctx, inviteToUUID)
		if err != nil {
	...
	if inviteToIdentity != nil {
		...
		identityResource, err = repo.ResourceRepository().Load(ctx, *inviteToIdentity.IdentityResourceID)
		if err != nil {
	...
	for i, invitation := range invitations {
		...
		if invitation.IdentityID != nil {
			identity, err := repo.Identities().Load(ctx, *invitation.IdentityID)
			if err != nil {
			...
			identities, err := repo.Identities().Query(account.IdentityFilterByUsername(*invitation.UserName))
			if err != nil {
				return errors.NewInternalError(ctx, err)
			}
			...
			users, err := repo.Users().Query(account.UserFilterByEmail(*invitation.UserEmail))
			if err != nil {
				return errors.NewInternalError(ctx, err)
			}
			...
			identities, err := repo.Identities().Query(account.IdentityFilterByUserID(*userID))
	...
	for _, invitation := range invitations {
		...
		error := repo.InvitationRepository().Create(ctx, inv)
		if error != nil {
		...
		for _, roleName := range invitation.Roles {
			...
			role, error := repo.RoleRepository().Lookup(ctx, roleName, resourceTypeName)
			...
			error = repo.InvitationRepository().AddRole(ctx, inv.InvitationID, role.RoleID)
			if error != nil {
				return errors.NewInternalError(ctx, error)
			}
		}
