...
func (s *permissionModelServiceBlackBoxTest) SetupSuite() {
	...
	permissionModelServiceRef := permissionModel.NewPermissionModelService(s.DB, s.Application)
	s.permissionService = permissionService.NewPermissionService(permissionModelServiceRef, s.Application)

	// Create a test "area" resource type
	role := s.setupResourceType(testResourceTypeArea, testAreaScopeName, uuid.NewV4().String())
	s.testAreaRole = *role

	// Create a test "workitem" resource type
	role = s.setupResourceType(testResourceTypeWorkItem, testWorkItemScopeName, uuid.NewV4().String())
	s.testWorkItemRole = *role

	// Create a test "workitemcomment" resource type
	role = s.setupResourceType(testResourceTypeWorkItemComment, testWorkItemCommentScopeName, uuid.NewV4().String())
